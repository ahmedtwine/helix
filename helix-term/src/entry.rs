...
pub type ConfigLoader = fn() -> Result<Config, ConfigLoadError>;
...
fn setup_logging(verbosity: u64) -> Result<()> {
    ...
    crate::logging::init_file(level, &helix_loader::log_file())?;

    Ok(())
}

pub async fn run(load_config: ConfigLoader) -> Result<i32> {
...
",
    ...
    if args.health {
        if let Err(err) = crate::health::print_health(args.health_arg) {
